---
name: archon-init
display_name: Archon Init Agent Skill
description: 负责检测 Lean 项目状态，初始化 .archon/ 文件结构，引导用户提供非形式化材料。
tags: [archon, init]
---
...
```bash
python3 <.archon-src>/scripts/sorry_analyzer.py . --format=summary
```

For per-file detail:
```bash
python3 <.archon-src>/scripts/sorry_analyzer.py . --format=markdown
```
...
When you advance the stage out of `init`, report to the user: "Init complete."
