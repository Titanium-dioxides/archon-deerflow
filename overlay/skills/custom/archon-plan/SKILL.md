---
name: archon-plan
display_name: Archon Plan Agent Skill
description: 负责读取非形式化材料，拆解任务，更新 task_pending 和 PROGRESS.md。
tags: [archon, planning]
---
...
1. Use the informal agent (`tools/archon_informal_agent.py`) to generate an informal proof sketch from an external model
...
1. Run `tools/archon_informal_agent.py` with a prompt describing the goal AND the constraint (e.g., "Prove X without using residue calculus, only tools available in Lean 4 Mathlib")
...
5. Reassign the task to the prover with the new informal proof
...
1. **Use the informal agent** — ask it: "Prove X without using [the missing infrastructure]. Only use tools available in Lean 4 Mathlib." Get a concrete alternative proof sketch.
...
4. **Check known-unavailable theorems** — if the missing infrastructure is in a known-unavailable domain, don't waste time looking for it. Focus on detours.
...
1. Check sorry count: `python3 <scripts>/sorry_analyzer.py <file> --format=summary`
...
- **Agent count = file count**: if 24 files need work, write 24 objectives — one per file. Do not artificially batch or limit the number of objectives.
